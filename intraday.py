...
def find_option_contracts(om,sym,spot):
    ...
    if x.empty:return x
    today=pd.Timestamp.now().normalize();start=today-pd.Timedelta(days=OPTIONS_DAYS)
    x=x[(x["expiry_dt"]>=start)&(x["expiry_dt"]<=today+pd.Timedelta(days=45))].copy()
    if x.empty:return x
    exps=sorted(x["expiry_dt"].dropna().unique());selected=[]
    for exp in exps:
...
def analyze(sym,tok,s,om):
    ...
    whole=whole_number_strategy(dd);nr7=nr7_strategy(dd);whole_bonus=WHOLE_BONUS if whole==direction else 0;nr7_bonus=NR7_BONUS if nr7==direction else 0;conflict=(4 if whole and whole!=direction else 0)+(4 if nr7 and nr7!=direction else 0)
    opt=option_flow(s,om,sym,entry);opt_bonus=opt["score"] if ((direction=="BUY" and opt["bias"]=="BULLISH") or (direction=="SELL" and opt["bias"]=="BEARISH")) else (-min(6,opt["score"]//2) if opt["bias"]!="NEUTRAL" else 0)
    final_score=int(min(100,max(0,tech_score+whole_bonus+nr7_bonus+opt_bonus-conflict)))
    sl=entry-atr if direction=="BUY" else entry+atr;t1=entry+1.5*atr if direction=="BUY" else entry-1.5*atr;t2=entry+2.5*atr if direction=="BUY" else entry-2.5*atr
    return {"symbol":sym,"direction":direction,"tech_score":int(tech_score),"score":final_score,"entry":entry,"sl":sl,"t1":t1,"t2":t2,"rsi":float(a.rsi),"volx":float(a.volx),"token":str(tok),"whole":whole or "-","nr7":nr7 or "-","whole_bonus":whole_bonus,"nr7_bonus":nr7_bonus,"options_bias":opt["bias"],"options_score":opt_bonus,"options_ratio":float(opt["ratio"]),"options_contracts":opt["contracts"]},None
...
def main():
    print(f"=== AI INTRADAY V7 FINAL {datetime.now(IST):%d %b %H:%M IST} ===");s=login();print("Loading master...");m=master();em=equity_master(m);om=option_master(m);print(f"NSE-EQ: {len(em)} | NFO contracts: {len(om)}")
    q=quotes(s,em.token.tolist())
    if q.empty:tg("⚠️ AI INTRADAY V7\nQuote API returned no data.");return
    q["symbolToken"]=q["symbolToken"].astype(str);q["ltp"]=pd.to_numeric(q["ltp"],errors="coerce");q["tradeVolume"]=pd.to_numeric(q["tradeVolume"],errors="coerce");q=q.dropna(subset=["symbolToken","ltp","tradeVolume"]);q=q[(q.ltp>=MIN_PRICE)&(q.tradeVolume>=MIN_VOL)].sort_values("tradeVolume",ascending=False).head(TOP_UNIVERSE);q=q.merge(em[["symbol","token"]].drop_duplicates("token"),left_on="symbolToken",right_on="token",how="left").dropna(subset=["symbol"]);print("Liquid universe:",len(q))
    stat={"candle":0,"feature":0,"daily":0,"ok":0,"score":0,"sl_small":0,"ipo_skip":0};res=[]
    for _,r in q.iterrows():
        sym=r.symbol;tok=r.symbolToken
        if sym in IPO_BLOCK:stat["ipo_skip"]+=1;continue
        z,why=analyze(sym,tok,s,om)
        if not z:
        ...
        print(f'{z["symbol"]:<18} {z["direction"]:<4} {z["score"]:>3} Tech {z["tech_score"]:>3} RSI {z["rsi"]:>5.1f} VolX {z["volx"]:>5.2f} NR7 {z["nr7"]:<4} Whole {z["whole"]:<4} Opt {z["options_bias"]}')
    res.sort(key=lambda x:(x["score"],x["tech_score"],x["volx"]),reverse=True);final_res=res[:30];sig=[x for x in final_res if x["score"]>=MIN_SCORE][:TOP_SIGNALS]
    msg=[f"⚡ AI INTRADAY V7 | {datetime.now(IST):%d-%b %H:%M IST}","",f"NSE-EQ: {len(em)} | Liquid: {len(q)} | Analysed: {stat['ok']}",f"Score ≥{MIN_SCORE}: {stat['score']} | Alert limit: {TOP_SIGNALS}","","🧠 Strategy Stack:","• 5m + 15m Technical","• Whole Number","• NR-7","• 3M Options Flow","• ATR Risk"]
    if sig:
        msg+=["","🔥 QUALIFYING SETUPS"]
        for i,z in enumerate(sig,1):
            msg.append(f'\n#{i} {z["symbol"]} {z["direction"]} {stars(z["score"])} ({z["score"]})\nEntry ₹{z["entry"]:.2f} | SL ₹{z["sl"]:.2f} | T1 ₹{z["t1"]:.2f} | T2 ₹{z["t2"]:.2f}\nTech {z["tech_score"]} | RSI {z["rsi"]:.1f} | Vol {z["volx"]:.2f}x\nWhole: {z["whole"]} (+{z["whole_bonus"]}) | NR7: {z["nr7"]} (+{z["nr7_bonus"]})\nOptions: {z["options_bias"]} (+{z["options_score"]}) | CE/PE Vol {z["options_ratio"]:.2f}')
    else:
        msg+=["","⚠️ NO QUALIFYING SETUP"]
        if final_res:
            msg+=["","👀 NEAR-MISS WATCHLIST"]+[f'#{i} {z["symbol"]} {z["direction"]} {stars(z["score"])} {z["score"]} | Tech {z["tech_score"]} | NR7 {z["nr7"]} | Whole {z["whole"]} | Opt {z["options_bias"]}' for i,z in enumerate(final_res[:8],1)]
        else:msg.append("No stock reached the watch score.")
    msg+=["","📊 Diagnostics:",f"Candle {stat['candle']} | Feature {stat['feature']} | Daily {stat['daily']} | SL-small {stat['sl_small']}",f"IPO skipped {stat['ipo_skip']}","","Alert-only • No auto orders"]
    out="\n".join(msg);print("\n"+out);tg(out)

if __name__=="__main__":
    try:main()
    except Exception as e:tg(f"❌ AI INTRADAY V7 ERROR\n{e}");raise
