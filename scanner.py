...
MIN_PRICE=50;MIN_AVG20=50000;VOL_THRESHOLD=1.8;QUOTE_BATCH=20;QUOTE_DELAY=1.5;CANDLE_DELAY=.35
...
SECTOR_MAP={k:v.split() for k,v in SECTOR_MAP.items()};SYMBOL_SECTOR={}
...
def login():
    ...
    a=SmartConnect(api_key=API_KEY);r=a.generateSession(CLIENT_ID,PASSWORD,pyotp.TOTP(TOTP_SECRET).now())
...
def candles(a,token,days=HISTORY):
    try:
        ...
        r=a.getCandleData({"exchange":"NSE","symboltoken":str(token),"interval":"ONE_DAY","fromdate":s.strftime("%Y-%m-%d %H:%M"),"todate":e.strftime("%Y-%m-%d %H:%M")})
        z=(r or {}).get("data") or []
        if not z:return pd.DataFrame()
        d=pd.DataFrame(z,columns=["date","open","high","low","close","volume"])
        for c in ["open","high","low","close","volume"]:d[c]=pd.to_numeric(d[c],errors="coerce")
        d["date"]=pd.to_datetime(d["date"]);return d.dropna().sort_values("date").drop_duplicates("date").set_index("date")
    except Exception as e:print("Candle:",e);return pd.DataFrame()
def comp(d):return d.iloc[:-1].copy() if d is not None and len(d)>1 else (d.copy() if d is not None else pd.DataFrame())
...
def rsi(s,n=14):
    d=s.diff();u=d.clip(lower=0);v=-d.clip(upper=0);au=u.ewm(alpha=1/n,adjust=False).mean();av=v.ewm(alpha=1/n,adjust=False).mean()
    return 100-100/(1+au/av.replace(0,np.nan))
def macd(s):
    m=ema(s,3)-ema(s,21);q=ema(m,9);return m,q,m-q
def atr(d,n=14):
    pc=d.close.shift();tr=pd.concat([d.high-d.low,(d.high-pc).abs(),(d.low-pc).abs()],axis=1).max(axis=1);return tr.rolling(n).mean()
def wk(d):return d.resample("W-FRI").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna()
def mo(d):return d.resample("ME").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna()

def index_status(d):
    d=comp(d)
    if len(d)<210:return {"status":"UNKNOWN","ret20":0}
    c=d.close;v=c.iloc[-1];d20=c.rolling(20).mean().iloc[-1];d50=c.rolling(50).mean().iloc[-1];d200=c.rolling(200).mean().iloc[-1];rr=(v/c.iloc[-21]-1)*100;rs=rsi(c,14).iloc[-1]
...
def sector_strength(data,nifty):
    nr=(nifty.close.iloc[-1]/nifty.close.iloc[-21]-1)*100 if len(nifty)>=25 else 0;res=[]
...
def daily(d):
    x=comp(d);c=x.close;e21=ema(c,21);e50=ema(c,50);e200=ema(c,200);a=atr(x,14);last=c.iloc[-1];av=a.iloc[-1]
    sw=x.low.tail(10).min();support=max(sw,e21.iloc[-1]*.985);sl=support-.5*av
    if sl>=last:sl=last-1.2*av
    risk=last-sl;rh=x.high.tail(20).max();breakout=last>=rh*.995;entry_low=max(e21.iloc[-1],support);entry_high=min(last,max(entry_low,rh*1.002));extended=last>entry_high*1.025
    valid=last>e21.iloc[-1]>e50.iloc[-1]>e200.iloc[-1] and not extended
    return {"valid":bool(valid),"setup":"BREAKOUT" if breakout else "PULLBACK/RECLAIM","entry_low":entry_low,"entry_high":entry_high,"sl":sl,"t1":last+2*risk,"t2":last+3*risk,"support":support,"rsi":rsi(c,14).iloc[-1],"ema21":e21.iloc[-1],"ema50":e50.iloc[-1],"ema200":e200.iloc[-1],"atr":av,"breakout":breakout,"high52":x.high.tail(252).max(),"ltp":last}
def fundamentals(sym):
    try:
        ...
        rev=next((x for x in ["Total Revenue","Operating Revenue"] if x in inc.index),None);prof=next((x for x in ["Net Income","Net Income Common Stockholders"] if x in inc.index),None)
...
def stars(s):return "★★★★★" if s>=85 else "★★★★☆" if s>=75 else "★★★☆☆" if s>=65 else "★★☆☆☆"
...
def main():
    ...
    if not nifty:raise RuntimeError("NIFTY token not found")
    nd=candles(a,m[nifty]["token"]);ns=index_status(nd)
    sd=candles(a,m[sensex]["token"]) if sensex else pd.DataFrame();ss=index_status(sd) if sensex else {"status":"UNAVAILABLE","ret20":0}
    print("MARKET:",ns["status"],ss["status"])
    if ns["status"]=="NEGATIVE" and ss["status"]=="NEGATIVE":
        print("Both NIFTY and SENSEX negative. BUY scan skipped.");return

    # 2) SECTOR DISCOVERY FIRST - fetch ALL mapped sector stocks, not pre-top-100
    ...
    if not goodsec:
        print("No strong sector found.");return
    goodnames={x["sector"] for x in goodsec}
    print("TOP SECTORS:",goodnames)

    # 3) ONLY STOCKS FROM TOP SECTORS -> liquidity filter
    ...
    def tech(z):
        di=z["di"];w=z["w"];sec=z["sec"];volx=z["df"].volume.iloc[-1]/max(1,z["df"].volume.rolling(20).mean().iloc[-1])
        return .25*(100 if sec["label"]=="FRESH" else 75)+.25*min(100,w["rsi"]*1.5)+.25*min(100,di["rsi"]*1.25)+.15*min(100,volx*25)+.10*(15 if di["breakout"] else 10)
    candidates=sorted(candidates,key=tech,reverse=True)[:TOP_FUNDAMENTAL]
    for z in candidates:
    ...
    if not final:print("No final BUY setup.");return
    ...
    for i,z in enumerate(final,1):
        d=z["df"];di=z["di"];w=z["w"];moq=z["mo"];f=z["fund"];volx=d.volume.iloc[-1]/max(1,d.volume.rolling(20).mean().iloc[-1])
