...
# ========== ANGEL ONE NSE BUY SCANNER V3.5.8 FIXED ==========
    ...
    for attempt in range(1,3):
        try:
            ...
            if data and data.get("status"):
                log("Angel Login OK - V3.5.8 FIXED")
                return obj
...
def main():
    log("="*70)
    log(" ANGEL ONE NSE BUY SCANNER V3.5.8 FIXED")
    log("="*70)
    ...
    if not candidates:
        send_telegram(f"No picks today - V3.5.8\nNSE: {len(nse_stocks)}\nVolume candidates: 0")
        return
    time.sleep(20)
    ...
    log("\n"+"="*70)
    log(f"DONE - V3.5.8 COMPLETE - Picks: {len(picks)}")
    log("="*70)
    if picks:
        log("\n🏆 FINAL RANKING\n")
...
if __name__=="__main__":
    try:
        ...
        log(f"\n❌ FATAL ERROR: {e}")
        send_telegram(f"❌ ANGEL ONE SCANNER V3.5.8 ERROR\n\n{e}")
